...
#splitting the white wine dataset
xTrainRed, xTestRed, tTrainRed, tTestRed = train_test_split(xRed, tRed, test_size=0.6)

scaler.fit(xTrainRed)
xTrainRed = scaler.transform(xTrainRed)
xTestRed = scaler.transform(xTestRed)

mlp = MLPRegressor(activation="relu", solver="adam", hidden_layer_sizes=(5,5),max_iter=1000,verbose=True,alpha=1e-6,random_state=42)
